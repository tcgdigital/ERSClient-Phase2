...
export class IncidentEntryComponent implements OnInit, OnDestroy {
    ...
    private validateEmergencyDate(control: FormControl): any {
        ...
        let validDate: Date;
        try {
            // The following statement is not cross-browser compatible
            // validDate = new Date(Date.parse(control.value));
            validDate = new Date(Date.parse(control.value.replace(/-/g, ' ')));
    ...
    public onBlue_EmergencyDate($event): void {
        if (!jQuery($event.currentTarget).prop('readonly')) {
            ...
            if (this.form.get('EmergencyDate').valid) {
                ...
                emergencyDate = new Date(Date.parse(this.form.get('EmergencyDate').value.replace(/-/g, ' ')));
    ...
    public onBlue_ReportedDate($event): void {
        if (!jQuery($event.currentTarget).prop('readonly')) {
            ...
            if (this.form.get('ReportedDate').valid) {
                ...
                reportedDate=new Date(Date.parse(this.form.get('ReportedDate').value.replace(/-/g, ' ')));
    ...
    loadDataIncidentViewPopup() {
        try {
            ...
            console.log(`function name: loadDataIncidentViewPopup, error: ${ex.message}`);
        }
    }
    ...
    createFlightModel(isFlightRelated: boolean): void {
        try {
            ...
            console.log(`function name: createFlightModel, error: ${ex.message}`);
        }
    }
    ...
    createIncidentModel(): void {
        try {
            ...
            console.log(`function name: createIncidentModel, error: ${ex.message}`);
        }
    }
